...
import {
  ...
  Clock, AlertTriangle, CreditCard, TrendingUp, FileText, X, Check
} from 'lucide-react';
import { adminFeeRecords, FeeRecord, adminUsers } from '@/lib/mockData';
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-box max-w-sm" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-3 p-5 border-t border-slate-100">
          ...
          <button onClick={() => onConfirm(record.id, amount)} className="btn btn-success flex-1 justify-center">
...
export default function FinanceDesk() {
  ...
  const handleMarkPaid = (id: string, amount: number) => {
    setRecords((prev) =>
      prev.map((r) =>
        r.id === id
          ? { ...r, status: 'Paid', paidAmount: amount, paidDate: new Date().toISOString().split('T')[0], receiptNo: `R-${Date.now().toString().slice(-5)}` }
          : r
      )
    );
    setPaidRecord(null);
  ...
  // Summary stats
  const totalCollected = records.filter(r => r.status === 'Paid').reduce((sum, r) => sum + (r.paidAmount ?? 0), 0);
  ...
  return (
    <div>
      <TopBar title="Finance Desk" subtitle="Generate challans, track payments & manage fee ledger" />
      ...
      <div className="p-4 md:p-6 space-y-5 max-w-7xl mx-auto">
        ...
        <div className="card overflow-hidden">
                        ...
                        <td className="hidden lg:table-cell">
                          {record.receiptNo ? (
                            <div className="text-xs text-slate-500">
                              <span className="font-mono">{record.receiptNo}</span>
                              <span className="block text-slate-400">{record.paidDate}</span>
                            </div>
                          ) : '—'}
                        </td>
                        <td>
                          <div className="flex items-center gap-1">
                            <button
                              onClick={() => setChallanRecord(record)}
                            ...
                            {record.status !== 'Paid' && (
                              <button
                                onClick={() => setPaidRecord(record)}
                                className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:bg-emerald-50 hover:text-emerald-600 transition-colors"
                                title="Mark Paid"
                              >
          ...
          <div className="px-4 py-3 border-t border-slate-100 bg-slate-50">
            <p className="text-xs text-slate-500">Showing {filtered.length} of {records.length} records</p>
          </div>
        </div>
      </div>
