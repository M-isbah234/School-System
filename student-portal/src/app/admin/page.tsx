...
import {
  ...
  CreditCard, Bell, Clock, CheckCircle, XCircle, AlertTriangle,
  ChevronRight, Activity, Layers
} from 'lucide-react';
import {
  schoolStats, adminUsers, adminFeeRecords, approvalRequests, notices
} from '@/lib/mockData';
import {
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white border border-slate-200 rounded-xl px-3 py-2 shadow-lg text-sm">
        ...
        {payload.map((p: any) => (
          <p key={p.name} style={{ color: p.color }}>{p.name}: <strong>{p.value}{p.name === 'rate' ? '%' : ''}</strong></p>
        ))}
      </div>
    );
  }
...
export default function AdminDashboard() {
  ...
  const [stats] = useState(schoolStats);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          ...
          <div className="lg:col-span-3 card p-5">
            ...
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={stats.attendanceTrend}>
                ...
                <Area type="monotone" dataKey="rate" name="rate" stroke="#6366f1" strokeWidth={2.5} fill="url(#attGrad)" dot={{ fill: '#6366f1', r: 4 }} activeDot={{ r: 6 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="lg:col-span-2 card p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => router.push('/admin/finance')} className="text-xs text-indigo-600 font-semibold hover:underline flex items-center gap-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="card p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => router.push('/admin/communication')} className="text-xs text-indigo-600 font-semibold hover:underline flex items-center gap-1">
            ...
            {recentNotices.map((notice) => (
              <div key={notice.id} className="p-3 rounded-xl bg-slate-50 border border-slate-100">
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  ...
                  <span className={`badge flex-shrink-0 ${notice.category === 'urgent' ? 'badge-rose' : notice.category === 'holiday' ? 'badge-amber' : notice.category === 'event' ? 'badge-blue' : 'badge-slate'}`}>
                    {notice.category}
                  </span>
                </div>
