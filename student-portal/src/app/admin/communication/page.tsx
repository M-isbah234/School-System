...
import {
  Bell, CheckCircle, XCircle, Clock, Plus, X, Check,
  Megaphone, FileText, UserCheck, ChevronDown, ChevronUp,
  AlertTriangle, Filter
} from 'lucide-react';
import { approvalRequests, ApprovalRequest, notices } from '@/lib/mockData';
...
// New Notice Modal
// ────────────────────────────────────────────
function BroadcastModal({ onClose, onSave }: { onClose: () => void; onSave: (n: Notice) => void }) {
  const [form, setForm] = useState({ title: '', content: '', category: 'info' as string });
      ...
      <div className="modal-box" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <select className="input cursor-pointer" value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))}>
          ...
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            ...
            {req.status === 'pending' && (
              <div className="flex gap-2 ml-auto">
                <button onClick={() => onReject(req.id)} className="btn btn-danger btn-sm">
                  <XCircle className="w-3.5 h-3.5" /> Reject
                </button>
                <button onClick={() => onApprove(req.id)} className="btn btn-success btn-sm">
...
export default function CommunicationPage() {
  ...
  return (
    <div>
      <TopBar title="Notices & Approvals" subtitle="Broadcast announcements and manage approval workflows" />
      ...
      <div className="p-4 md:p-6 space-y-5 max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowBroadcast(true)} className="btn btn-primary">
